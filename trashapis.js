...
function afvalkalenderSudwestFryslan(postcode, housenumber, street, country) {
    return AfvalkalendersSudwestFryslan(postcode, housenumber, country);
}
...
/**
 * Implemenation of afvalkalender Sudwest Fryslan
 */
function AfvalkalendersSudwestFryslan(postcode, housenumber, country) {
    console.log("Checking afvalkalender Sudwest Fryslan");
    ...
    var retrieveIdentificationRequest = httpsPromise({
        hostname: 'afvalkalender.sudwestfryslan.nl',
        path: `/adressen/${postcode}:${housenumber}`,
        method: 'GET'
    });
    ...
    return new Promise(function(resolve, reject)
    {
        retrieveIdentificationRequest.then(function(response)
        {
            ...
            console.log('response addresses:', JSON.stringify(result, null, 2));

            var identificatie = result[0].bagid;
            
            var retrieveAfvalstromen  = httpsPromise({
                hostname: 'afvalkalender.sudwestfryslan.nl',
                path: `/rest/adressen/${identificatie}/afvalstromen`,
                method: 'GET'
            });
            retrieveAfvalstromen.then(function(response)
            {
                console.log('response afvalstromen:', JSON.stringify(response.body, null, 2));
                
                var afvalstroom_id_to_trash_type = {
                    3 : 'PAPIER',
                    1 : 'REST',
                    2 : 'GFT',
                    4 : 'PLASTIC',
                ...
                var afvalstromen = response.body;
                for (let i in afvalstromen) {
                    const afvalstroom = afvalstromen[i];

                    if(typeof afvalstroom_id_to_trash_type[afvalstroom.id] === 'undefined') {
                        var afvalstroom_title = afvalstroom.title.toLowerCase();

                        if(
                            afvalstroom_title.indexOf('papier') !== -1 | 
                            afvalstroom_title.indexOf('karton') !== -1
                        ) {
                            afvalstroom_id_to_trash_type[afvalstroom.id] = 'PAPIER';
                        }

                        if(
                            afvalstroom_title.indexOf('rest') !== -1 
                        ) {
                            afvalstroom_id_to_trash_type[afvalstroom.id] = 'REST';
                        }


                        if(
                            afvalstroom_title.indexOf('gft') !== -1 |
                            afvalstroom_title.indexOf('groen') !== -1
                        ) {
                            afvalstroom_id_to_trash_type[afvalstroom.id] = 'GFT';
                        }

                        if(
                            afvalstroom_title.indexOf('plastic') !== -1
                        ) {
                            afvalstroom_id_to_trash_type[afvalstroom.id] = 'PLASTIC';
                        }

                        if(
                            afvalstroom_title.indexOf('grof') !== -1
                        ) {
                            afvalstroom_id_to_trash_type[afvalstroom.id] = 'GROF';
                        }

                        if(
                            afvalstroom_title.indexOf('textiel') !== -1
                        ) {
                            afvalstroom_id_to_trash_type[afvalstroom.id] = 'TEXTIEL';
                        }

                        if(
                            afvalstroom_title.indexOf('kerst') !== -1
                        ) {
                            afvalstroom_id_to_trash_type[afvalstroom.id] = 'KERSTBOOM';
                        }
                    }
                }
                console.log('Afvalstromen to trashtype:', JSON.stringify(afvalstroom_id_to_trash_type, null, 2));

                var currentYear = new Date().getFullYear();
                var retrieveDatesCurrentYear  = httpsPromise({
                    hostname: 'afvalkalender.sudwestfryslan.nl',
                    path: `/rest/adressen/${identificatie}/kalender/${currentYear}`,
                    method: 'GET'
                });
                var retrieveDatesNextYear  = httpsPromise({
                    hostname: 'afvalkalender.sudwestfryslan.nl',
                    path: `/rest/adressen/${identificatie}/kalender/${currentYear + 1}`,
                    method: 'GET'
                });
                Promise.all([retrieveDatesCurrentYear, retrieveDatesNextYear]).then(data => {
                    var dates_to_convert = [];
                    for (let i in data[0].body) {
                        dates_to_convert.push(data[0].body[i]);
                    }
                    for (let i in data[1].body) {
                        dates_to_convert.push(data[1].body[i]);
                    }
                    console.log('Dates to convert:', JSON.stringify(dates_to_convert, null, 2));
                    ...
                    for (let i in dates_to_convert) {
                        // Skip ID's we don't know the type of
                        var trash_type = afvalstroom_id_to_trash_type[dates_to_convert[i].afvalstroom_id];
                        if(trash_type !== 'undefined') {
                            var date = formatDate(dates_to_convert[i].ophaaldatum);
                            if(dates[trash_type].indexOf(date) === -1) {
                                dates[trash_type].push(date);
                            }
                        }
                    }
            ...
            .catch(function(error)
            {
                console.log("retrieve afvalstromen rejected");
                console.log(error);
                reject(error);
            });
        }).catch(function(error)
