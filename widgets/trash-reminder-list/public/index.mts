...
class WidgetScript {
  ...
  private convertSettings(settings: Record<string, unknown>): WidgetSettings {
    const mappedTypes: Record<TrashType, boolean> = {} as Record<TrashType, boolean>;
    Object.keys(settings).forEach((key) => {
      if (key.startsWith('type-')) {
        const trashType = key.replace('type-', '') as TrashType;
        mappedTypes[trashType] = settings[key] as boolean;
      }
    });
  ...
  public async Init() {
    try {
      ...
      for (let index in collectionInfo) {
        ...
        if (this.settings.displayTypes[trashItem.type] === false) continue;
        ...
        if (this.settings.layoutType === 'large') {
          ...
          <div class="trash-item-container">
            <span class="dot-large dot-type-${trashItem.type}"><img src="${trashItem.icon ?? `trash-${trashItem.type}.svg`}" /></span>
            <p class="trash-date-title trash-date">${displayDate}</p>
            ${trashItem.isCleaned ? '<span class="cleaning-icon">🫧</span>' : ''}
          </div>`;
        } else {
          const trashTypeText = this.homey.__(`widgets.trashType.${trashItem.type}`);
          rowHeight = 57;
          itemsPerRow = 2;

          tbody.innerHTML += `
          <div class="trash-item-container-compact">
            <span class="dot-compact dot-type-${trashItem.type}"><img src="${trashItem.icon ?? `trash-${trashItem.type}.svg`}" /></span>
